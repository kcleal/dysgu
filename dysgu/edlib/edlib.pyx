...
def align(query, target, mode="NW", task="distance", k=-1, additionalEqualities=None):
    ...
    cconfig = edlibDefaultAlignConfig()

    if k is not None: cconfig.k = k

    if mode == 'NW': cconfig.mode = EDLIB_MODE_NW
    if mode == 'HW': cconfig.mode = EDLIB_MODE_HW
    if mode == 'SHW': cconfig.mode = EDLIB_MODE_SHW

    if task == 'distance': cconfig.task = EDLIB_TASK_DISTANCE
    if task == 'locations': cconfig.task = EDLIB_TASK_LOC
    if task == 'path': cconfig.task = EDLIB_TASK_PATH

    cdef bytes tmp_bytes;
    cdef char* tmp_cstring;
    cdef EdlibEqualityPair* c_additionalEqualities = NULL
    if additionalEqualities is None:
        cconfig.additionalEqualities = NULL
        cconfig.additionalEqualitiesLength = 0
    else:
        c_additionalEqualities = <EdlibEqualityPair*> PyMem_Malloc(len(additionalEqualities)
                                                                          * cython.sizeof(EdlibEqualityPair))
    ...
    with nogil:
        cresult = edlibAlign(cquery, query_len, ctarget, target_len, cconfig)
    ...
    if cresult.alignment:
        ccigar = edlibAlignmentToCigar(cresult.alignment, cresult.alignmentLength,
